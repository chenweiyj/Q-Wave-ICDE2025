...
class Node:
    ...
    def direct_broadcast(self, message):
        ...
        sqrt_len = int(math.sqrt(len(self.shard_addresses)))
        with self.meesages_to_send_lock:
            self.messages_to_send[self.shard_addresses[my_index]].append(message)
            self.message_queue.put((self.shard_addresses[my_index]))
            for i in range(sqrt_len+5):
                next_index = (my_index + (i+1) * sqrt_len ) % len(self.shard_addresses)
 ...
 #       # Get the index of this node in the shard_addresses
        if  message['retransmit']==0:
            return
        my_index = self.shard_addresses.index(self.address)
        sqrt_len = int(math.sqrt(math.sqrt(len(self.shard_addresses))))
         ...
         #   self.retransmit(message)
            ...
            with self.meesages_to_send_lock:
                if hash_content_ not in self.retransit_sent["1"]:
                    ...
                    self.message_queue.put((self.shard_addresses[my_index]))
                    for i in range (sqrt_len+5):
                        next_index = (my_index + i+1) % len(self.shard_addresses)
                        self.messages_to_send[self.shard_addresses[next_index]].append(message)
                        self.message_queue.put((self.shard_addresses[next_index]))
        ...
        for i in range(sqrt_len+1):
            next_index = (my_index + 1 + i) % len(self.shard_addresses)
            nodes_to_contact.append(self.shard_addresses[next_index])
            #print (f"level 1 {my_index} -> {next_index}")
        for address in nodes_to_contact:
            if address != self.address:  # Avoid sending the message to itself
                with self.meesages_to_send_lock:
                    self.messages_to_send[address].append(message)
                    self.message_queue.put((address))"""
    ...
    def activate_a_new_instance (self,node_id, address, coordinator_address):
        tt1=Node(node_id, address, coordinator_address)
        global global_instance_list
        global_instance_list.append(tt1)
...
if __name__ == "__main__":
    ...
    tt =  Node(node_id, address, coordinator_address)
    global_instance_list.append(tt)
    print("started")